...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[Metricool Instagram] Publish response:', publishRes.status, publishText.slice(0, 300))
    ...
    if (!publishRes.ok) {
      console.error('[Metricool Instagram] Publish error:', publishData)
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: 'Publicado en Metricool para Instagram',
      postId: publishData.id,
    })
  } catch (error) {
    console.error('[Metricool Instagram] Error:', error)
    return NextResponse.json(
